...
function EditDonor(){
    ...
    const Update = async () =>{
        ...
        const response = await fetch(`https://blood-donation-backend-a4xh.onrender.com/${id}`, requestOptions);
    ...
    const getDataById = async (id) =>{
        const response = await fetch('https://blood-donation-backend-a4xh.onrender.com/getDonorById/'+id);
        const data = await response.json();
        setName(data.name);
        setEmail(data.email);
